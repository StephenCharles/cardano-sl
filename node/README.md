...
### Building Cardano with Nix

Cardano SL binaries can be obtained from the IOHK binary cache repositories
entirely using the Nix package manager.

Installation of the Nix package manager is not covered here -- for that, please
refer to [its download page](https://nixos.org/nix/download.html).

You can validate a correct Nix installation by checking that the nix-build binary
is available.

Once that is done, building Cardano SL (or, actually, obtaining it from the IOHK's
build caches) can be performed by:

    git checkout cardano-sl-1.0
    scripts/build/cardano-sl.nix.sh
